...
#To DO:
#Check Xray is getting results, and being shown correctly
#What will we show in CT cases?
...
def result(page):                       #Creates Results Page 
    ...
    takez.place(relx=.5,rely=.75)
    resLabel.place(relx=.4,rely=.0)
    
    

    #
    if sonuclarxray:
        h = Label(resultcanvas,bg="#909090", fg="red",font =("Montserrat", 14), text=str(sonuclarxray))
        h.place(relx=.24,rely=.5)
        panel.place(relx=.60,rely=.2)
       
    if sonuclarct:
        c = Label(resultcanvas,bg="#909090", fg="red",font =("Montserrat", 14), text=str(translate(sonuclarct)))
        c.place(relx=.7,rely=.2)
    sonuclarct.clear()
    sonuclarxray.clear()
    ...
    # Brescia check part
        ...
        tedaviz="Remdesivir (Eğer mevcut değilse:  Lopinavir/Ritonavir \n veya Boosted/Darunavir) +  Klorokin veya Hidroksiklorin \n+ Deksametazonu Düşün  (Yaş, komorbidite ve bilişsel bozulma \n göz önüne alınarak) + Tocilizumab'ı Düşün.  (Dahil etme kriterleri: 1. Yüksek SARS-CoV2 viral yükü \n riskinin düşük olması: >72 saat ateş olmaması, semptomların \n >7 gündür olması 2.Interlökin-6>40pg/ml \n veya yüksek plazmatik D-Dimer ve/veya yüksek CRP \n ve/veya  yüksek ferritin ve/veya yüksek ﬁbrinojen  seviyesi)"
        cevab = ""
        tedavi=""
        if (solunumd.get() + solunumsayid.get() + grafid.get() + oksijend.get())==0:
            cevab ="Hastayı SpO2 ile monitorize etmeye \n devam et ve klinik olarak değerlendir"
            tedavi = "Standart Tedavi \n Lopinavir / Ritonavir (veya Boosted/Darunavir) + Klorokin veya Hidroksiklorokin"

        if (solunumd.get() + solunumsayid.get() + grafid.get() + oksijend.get())==1:
            cevab ="Oksijen desteği ekle. Hastayı SpO2 \n ile monitorize etmeye devam et ve klinik olarak değerlendir"
            tedavi = "Standart Tedavi \n Lopinavir / Ritonavir (veya Boosted/Darunavir) \n  Klorokin veya Hidroksiklorokin"
        if (solunumd.get() + solunumsayid.get() + grafid.get() + oksijend.get())==2:
            cevab ="Göğüs grafisi çek, gaz analizi yap\n Oksijen desteği ekle \n Hastayı SpO2 ile monitorize etmeye devam et ve klinik olarak değerlendir"
            tedavi = "Standart Tedavi + Deksametazonu düşün \n (Yaş, komorbidite ve bilişsel bozulma göz önüne alınarak)"

        if (solunumd.get() + solunumsayid.get() + grafid.get() + oksijend.get())>2:
            cevab ="Her 2 günde bir göğüs grafisi çek ve \n günde 2 kere gaz analizi yap. Hastayı SpO2 ile \n monitorize etmeye devam et ve klinik olarak değerlendir"
            tedavi = "Standart Tedavi \n Deksametazonu düşün \n (Yaş, komorbidite ve bilişsel bozulma göz önüne alınarak) \n Tocilizumab'ı düşün (Dahil etme kriterleri:\n 1. Yüksek SARS-CoV2 viral yükü riskinin düşük olması: \n >72 saat ateş olmaması\n semptomların >7 gündür olması \n 2.Interlökin-6>40pg/ml veya yüksek plazmatik DDimer ve/veya yüksek CRP \n ve/veya yüksek ferritin ve/veya yüksek ﬁbrinojen seviyesi)"

        
            if prone.get()==1:
                cevab = "Yüksek derecede karmaşıklık"
                tedavi = "Remdesivir (Eğer mevcut değilse:  Lopinavir/Ritonavir \n veya Boosted/Darunavir) +  Klorokin veya Hidroksiklorin \n+ Deksametazonu Düşün  (Yaş, komorbidite ve bilişsel bozulma \n göz önüne alınarak) + Tocilizumab'ı Düşün.  (Dahil etme kriterleri: 1. Yüksek SARS-CoV2 \n viral yükü  riskinin düşük olması: >72 saat ateş olmaması, semptomların \n >7 gündür olması 2.Interlökin-6>40pg/ml \n veya yüksek plazmatik D-Dimer  ve/veya yüksek \n CRP ve/veya  yüksek ferritin ve/veya yüksek ﬁbrinojen  seviyesi)"
            
            elif cpap.get()==0:
                tedavi = "Cpap devam ve Standart Tedavi \n Deksametazonu düşün \n (Yaş, komorbidite ve bilişsel bozulma göz önüne alınarak) \n Tocilizumab'ı düşün (Dahil etme kriterleri:\n 1. Yüksek SARS-CoV2 viral yükü riskinin düşük olması: \n >72 saat ateş olmaması\n semptomların >7 gündür olması \n 2.Interlökin-6>40pg/ml veya yüksek plazmatik DDimer ve/veya yüksek CRP \n ve/veya yüksek ferritin ve/veya yüksek ﬁbrinojen seviyesi)" 
                ...
                cevab="Sedasyonu minimize etmeyi dene(RASS- RichmondAjitasyon Sedasyon Skalası-1 ile 0 ) \n Günlük SBT(SpontanBreathing Trial) uygula."
                tedavi = tedaviz
            elif nmba.get()==0:
                cevab = "Sedasyonu minimize etmeyi dene(RASS- RichmondAjitasyon Sedasyon Skalası-1 ile 0 )"
                tedavi = tedaviz
            elif prone.get()==0:
                cevab = "En iyi PEEP(Positive End Expratory Pressure) ve Kompliyans hesaplamasını yap.\n NMBA'yi askıya almayı dene \n Volum seviyesini optimize et."
                tedavi = tedaviz
            
    
        progzlabel = Label(resultcanvas,bg="#909090", fg="blue",font=("Montserrat", 14), text=cevab)
        progzlabel.place(relx=.0,rely=.17)
        progcevab = Label(resultcanvas,bg="#909090", fg="red",font=("Montserrat", 14), text=tedavi)
        progcevab.place(relx=.0,rely=.37)
    ...
    #img = ImageTk.PhotoImage(Image.open(img_names[0]))
    #resultcanvas.create_image(250,200,image=img)
...
def sempf():                                #Creates Symptoms Page
    ...
    nextiz.place(relx=.35,rely=.75)
    skipz2.place(relx=.65, rely=.77)
...
diagb = PhotoImage(file="diagb.png") # Big diagnose button img 
progb = PhotoImage(file="progb.png")

xrayimg = PhotoImage(file="xrayimg.png")
ctimg = PhotoImage(file="ctimg.png")

corimg = PhotoImage(file="cclogo.png")
corimg2 = PhotoImage(file="CB2.png")

home = PhotoImage(file="home.png")
prog = PhotoImage(file="prog.png")
diag = PhotoImage(file="diag.png")

home_clicked = PhotoImage(file="home_clicked.png")
prog_clicked = PhotoImage(file="prog_clicked.png")
diag_clicked = PhotoImage(file="diag_clicked.png")

skip = PhotoImage(file="skip.png")
nexti = PhotoImage(file="next.png")
take = PhotoImage(file="takeagain.png")
nextQ = PhotoImage(file="nextQ.png")

#canvas
resultcanvas = Canvas(root,height=400 ,width=750 ,bg="#909090",borderwidth=0,highlightthickness=0)


#Bresica questions
solunum = Checkbutton(resultcanvas,variable=solunumd, bg="#909090",font=("Montserrat", 14),text="Hastanın solunum sıkıntı veya kesik kesik konuşması var mı?",)
solunum_sayi = Checkbutton(resultcanvas,variable=solunumsayid, bg="#909090",font=("Montserrat", 14), text="Solunum sayısı >22 mi?")
oksijen = Checkbutton(resultcanvas, variable=oksijend, bg="#909090",font=("Montserrat", 14),text="PaO2 <65mmHg yada SpO2 <%90 mı?")
grafi = Checkbutton(resultcanvas,variable=grafid, bg="#909090",font=("Montserrat", 14), text="Göğüs grafisinde belirgin kötüleşme var mı?")
secondp = Checkbutton(resultcanvas,variable=cpap, bg="#909090",font=("Montserrat", 14),text="CPAP kullanılmasına rağmen hala \n 2'den fazla kriter pozitif mi?")
cmvb = Checkbutton(resultcanvas,variable=cmv, bg="#909090",font=("Montserrat", 14),text="CMV (Continuous Mandatory Ventilation)?")
paob = Checkbutton(resultcanvas,variable=pao, bg="#909090",font=("Montserrat", 14),text="PaO2/FiO2<150mmHg? (Parsiyel O2 basıncı/ Fraction of Inspired Oxygen)")
nmbab = Checkbutton(resultcanvas,variable=nmba, bg="#909090",font=("Montserrat", 14), text="NMBA (Neuromuscular blockade)?")
proneb = Checkbutton(resultcanvas,variable=prone, bg="#909090",font=("Montserrat", 14), text="Yüzüstü pozisyon? iNO(inhaled Nitrik Oksit)? \n ECLS(Extracorporeal Life Support)?")


#Semp questions
atesb= Checkbutton(resultcanvas, bg="#909090",font=("Montserrat", 14),text="Ateşim 38C üstünde",variable=ates)
oksurukb= Checkbutton(resultcanvas, bg="#909090",font=("Montserrat", 14),text="Öksürüğüm var",variable=oksuruk)
nefesb= Checkbutton(resultcanvas, bg="#909090",font=("Montserrat", 14),text="Nefes darlığım var ",variable=nefes)
hastalıkb= Checkbutton(resultcanvas, bg="#909090",font=("Montserrat", 14),text="Aşağıdaki hastalık veya durumlardan herhangi birine sahibim \n-İmmun yetmezlik \n-Hipertansiyon \n-Kemoterapi \n-Diyabet\n-Astım  ",variable=hastalık)
temasb= Checkbutton(resultcanvas, bg="#909090",font=("Montserrat", 14),text="COVID-19 için test yapılmış ve sonucu pozitif gelmiş kişilerle temasım oldu",variable=temas)


#Labels
entübe = Label(resultcanvas, text="Lütfen Hastayı entübe ediniz", fg="red", bg="#909090", font=("Montserrat", 23))
answerlabel=Label(resultcanvas, bg="#909090",font=("Montserrat", 16), text="CEVAPLARINIZ KAYDEDİLMİŞTİR LÜTFEN \n SONRAKİ SAYFAYA GEÇİNİZ",fg="red")
resLabel = Label(resultcanvas, text="RESULTS", fg="white", bg="#909090", font=("Montserrat", 16))
riskLabel = Label(resultcanvas, text="Risk of COVID-19:", fg="white", bg="#909090", font=("Montserrat", 16))
radioLabel = Label(resultcanvas, text="Radiological:", fg="white", bg="#909090", font=("Montserrat", 16))
sympLabel = Label(resultcanvas, text="Symptoms:", fg="white", bg="#909090", font=("Montserrat", 16))
...
risklabel1 = Label(resultcanvas,bg="#909090",font=("Montserrat", 15), text="High risk",fg="red")
risklabel2 = Label(resultcanvas,bg="#909090",font=("Montserrat", 15), text="Normal risk",fg="red")
risklabel3 = Label(resultcanvas,bg="#909090",font=("Montserrat", 15), text="Low risk",fg="red")
...
nextctnormal = Button(root, image = nexti, command = Bscore,borderwidth=0,highlightthickness=0)
...
ctz = Button(root, image = ctimg, command= lambda:addimage("ct"),borderwidth=0,highlightthickness=0)
...
ctdata = np.expand_dims(ctdata,axis=0)
...
import pydicom
import math
ctdata = unlabeledProcessing(img_names[0])


# In[90]:


modelct.predict(ctdata)
...
solunumd.get() + solunumsayid.get() + grafid.get() + oksijend.get()
